  ...
  static async create(locale: Locale, project: Omit<Project, 'id' | 'created_at' | 'updated_at' | 'locale' | 'technologies'>, technologies: string[] = []): Promise<Project> {
    return await transaction(async (client) => {
      ...
      const result = await q<Project>(
        ...
        RETURNING *`,
        [locale, project.title, project.description, project.image_url, project.gradient, project.border_color, project.glow_color, project.live_url, project.github_url, project.featured, project.order_index]
      );
  ...
  static async update(id: string, project: Partial<Project>, technologies?: string[]): Promise<Project> {
    return await transaction(async (client) => {
      ...
      await q(
        ...
        WHERE id = $11`,
        [project.title, project.description, project.image_url, project.gradient, project.border_color, project.glow_color, project.live_url, project.github_url, project.featured, project.order_index, id]
      );
